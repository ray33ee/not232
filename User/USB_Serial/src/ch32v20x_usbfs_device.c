...
/* CDC line coding (7 bytes) default: 115200 8N1 */
static volatile uint8_t g_line_coding[7] = {
            ...
            case USBFS_UIS_TOKEN_OUT:
                switch ( intst & ( USBFS_UIS_TOKEN_MASK | USBFS_UIS_ENDP_MASK ) )
                {
                    ...
                    case USBFS_UIS_TOKEN_OUT | DEF_UEP0:
                        ...
                        if ( intst & USBFS_UIS_TOG_OK )
                        {
                            if ( ( USBFS_SetupReqType & USB_REQ_TYP_MASK ) != USB_REQ_TYP_STANDARD )
                            {
                                ...
                                {
                                      /* Host sent 7 bytes into EP0 buffer */
                                        for(uint8_t i = 0; i < 7; i++)
                                            g_line_coding[i] = USBFS_EP0_Buf[i];

                                        /* No UART init here (we¡¯re pure USB CDC) */
                    ...
                    case USBFS_UIS_TOKEN_OUT | DEF_UEP2:
                        {
                            /* Toggle + accept next packet like original */
                            USBOTG_FS->UEP2_RX_CTRL ^= USBFS_UEP_R_TOG;

                            uint16_t rlen = USBOTG_FS->RX_LEN;
                            usbSerial_on_ep2_out(rlen);

                            /* Keep endpoint armed for more OUT */
                            USBOTG_FS->UEP2_RX_CTRL = (USBOTG_FS->UEP2_RX_CTRL & ~USBFS_UEP_R_RES_MASK) | USBFS_UEP_R_RES_ACK;
                            break;
                        }
                        break;
                    ...
                    if( USBFS_SetupReqType & USB_REQ_TYP_CLASS )
                    {
                        ...
                        switch( USBFS_SetupReqCode )
                        {
                            case CDC_GET_LINE_CODING:
                                pUSBFS_Descr = (uint8_t *)g_line_coding;
                                len = 7;
                                break;
