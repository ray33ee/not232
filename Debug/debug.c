...
void start_timer_us_u32(uint32_t duration) {
    uint32_t i;

    SysTick->SR &= ~(1 << 0);
    i = (uint32_t)duration * p_us;
...
void start_timer_us_u8(uint8_t duration) {
    //Use a u8 * 20 microsecond timeout - this means a timeout of between 0-5000 ish microseconds
    start_timer_us_u32((uint32_t)duration * 20);
}

uint32_t has_timer_timeout() {
